...
export const ProductListingComponent: FC<Props> = props => {
  ...
  const categories = props.item.elements.productCategory?.value?.map(term => term.codename).join(", ")

  useEffect(() => {
    const getProducts = async () => {
      const response = await fetch(`/api/products?preview=${isPreview}&category=${categories}&language=${router.locale}`);
